# 基于 sam2_main4_joint_init_seed_judge_vos：
# - 先做 seed judge
# - TRUST seed 执行联合初始路径 + VOS
# - 若分割过程中 mask 突然消失，则在上一帧重采样 seed，并重新走 joint+judge 后再分割
# - 增加 max_segmented_seeds，避免递归扩散导致无限循环
import argparse
import os
import shutil
import tempfile
...
def get_args():
    ...
    parser.add_argument("--output_dir", default="./bv_seg_output_joint_seed_judge_respawn")
...
def seed_from_local_mask(axis, box, global_frame_idx, mask_2d, vol_shape):
...
def vos_track_with_multi_init_and_respawn(
    video_predictor,
    vol_man,
    axis,
    box,
    ...
    tmp_dir = tempfile.mkdtemp(prefix="sam2_vos_joint_respawn_", dir=vos_tmp_root)
    ...
    try:
        with torch.inference_mode(), torch.autocast(str(vol_man.device), dtype=torch.bfloat16):
            ...
            for gidx in idx_list:
                ...
                vol_man.update_global_mask(m_u8, global_update_axis, (gidx, *box[1:]))
                prev_nonempty_mask = m_u8
            ...
            if used == 0:
                ...
                m0u = m0.astype(np.uint8)
                vol_man.update_global_mask(m0u, global_update_axis, (g0, *box[1:]))
                prev_nonempty_mask = m0u
                prev_global_idx = g0
                ...
                if mm.sum() == 0:
                    if enable_respawn and prev_nonempty_mask is not None and prev_global_idx is not None:
                        if int(prev_nonempty_mask.sum()) >= int(min_respawn_mask_area):
                            new_seed = seed_from_local_mask(
                                axis=axis,
                                box=box,
                                global_frame_idx=prev_global_idx,
                                mask_2d=prev_nonempty_mask,
                                vol_shape=vol_man.shape,
                            )
                            if new_seed is not None and new_seed not in spawned_set:
                                spawned_set.add(new_seed)
...
def run_segmentation_with_seed_judge_respawn():
    args = get_args()
    ...
    pbar = tqdm(total=len(pending), desc="Seed judge + VOS + respawn")
    while len(pending) > 0:
        ...
        new_fw = vos_track_with_multi_init_and_respawn(
        ...
        new_bw = vos_track_with_multi_init_and_respawn(
            video_predictor=video_predictor,
            vol_man=vol_man,
            axis=best_axis,
            box=box,
    ...
    print("=" * 80)
    print(f"Trusted segmented seeds: {trust_cnt}")
    print(f"Rejected seeds: {reject_cnt}")
    print(f"Respawned seeds queued: {spawned_cnt}")
...
if __name__ == "__main__":
    run_segmentation_with_seed_judge_respawn()
